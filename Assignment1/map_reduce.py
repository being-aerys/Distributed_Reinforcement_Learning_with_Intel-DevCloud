...
if len(sys.argv) < 2:
    ...
    cpus = int(sys.argv[1]) #second argument
print('running map reduce with %d cpus' % cpus)
ray.init(num_cpus=int(cpus), include_webui=False, ignore_reinit_error=True, redis_max_memory=1000000000,
...
def map_parallel(function, xs):
    ...
    if not isinstance(xs, list):#-------------------------------> xs should be a list of arguments
        raise ValueError('The xs argument must be a list.')

    if not hasattr(function, 'remote'):#-------------------------------------if function argument does not have an attribute named "remote", then its not remote

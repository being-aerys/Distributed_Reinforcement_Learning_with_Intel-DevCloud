...
# **EXERCISE:** The loop below takes too long. Parallelize the four passes through the loop by turning `load_data`, `normalize_data`, `extract_features`,
# and `compute_loss` into remote functions and then retrieving the losses with `ray.get`.
#
...
for filename in ['file1', 'file2', 'file3', 'file4']:
    ...
    normalized_data = normalize_data.remote(data)#------------->without the need to get the actual value of the load_data(), we just pass the id as the arg
...
losses_actual = ray.get(losses)#-----------------Added here
print('The losses are {}.'.format(losses_actual) + '\n')
...
end_time = time.time()
duration = end_time - start_time
assert not any([isinstance(results, ray.ObjectID) for result in
...
result_ids = [f.remote(i) for i in range(6)]#--------------------------------->Parallelly launching six tasks
# Get one batch of tasks. Instead of waiting for a fixed subset of tasks, we
# should instead use the first 3 tasks that finish.

ready_ids, remaining_ids = ray.wait(result_ids, num_returns=3, timeout=100)#------------> Getting the first three tasks



initial_results = ray.get(ready_ids[:3])
...
remaining_results = ray.get(remaining_ids[:3])
